...
const { IS_NOT_TEST_PERIOD, LOGIN_REQUIRED } = require('../../../../errors');
...
const handleAccessProblem = asyncHanlder(async (req, res, next) => {
  ...
  const { parentId:parent, parentType } = problem;
  ...
  if (!isAssigned(user, parent))return next(IS_NOT_TEST_PERIOD);
  next();
});
